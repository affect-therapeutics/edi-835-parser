...
for file in files:
    ...
    remit_fin_info_df.to_csv(
        f"{output_dir}/payment_financial_info/{file}", sep="|", index=False
    )
    ...
    remit_service_lines_df.to_csv(
        f"{output_dir}/remit_service_lines/{file}", sep="|", index=False
    )
    ...
    remit_adjustments_df.to_csv(
        f"{output_dir}/remit_adjustments/{file}", sep="|", index=False
    )
    ...
    if (
        len(service_line_remarks_df) > 0
        and service_line_remarks_df.isnull().values.any()
    ):
    ...
    #
    service_line_rendering_providers_df = (
        transaction_set.build_service_line_rendering_providers()
    )
    if (
