...
for file in files:
	...
	remit_remarks_adjudications_df.to_csv(f'{output_dir}/remit_remarks_adjudications/{file}',
											sep='|', index=False)

	provider_adjustments_df = transaction_set.build_provider_adjustments()
	provider_adjustments_df.insert(0, 'file_name', file, False)
	provider_adjustments_df.to_csv(f'{output_dir}/provider_adjustments/{file}',
											sep='|', index=False)
	...
	service_line_adjustment_df.to_csv(f'{output_dir}/service_line_adjustments/{file}',
										sep='|', index=False)
	...
	service_line_remarks_df.to_csv(f'{output_dir}/service_line_remarks/{file}',
											sep='|', index=False)
	#
	service_line_rendering_providers_df = transaction_set.build_service_line_rendering_providers()
	if len(service_line_rendering_providers_df) > 0 and service_line_rendering_providers_df.isnull().values.any():
		service_line_rendering_providers_df.insert(0, 'file_name', file, False)
	else:
		service_line_rendering_providers_df['file_name'] = ''
	service_line_rendering_providers_df.to_csv(f'{output_dir}/service_line_rendering_providers/{file}',
													sep='|', index=False)
