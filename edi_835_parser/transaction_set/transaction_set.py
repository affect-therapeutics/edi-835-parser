...
class TransactionSet:
    ...
    def build_remits(self) -> pd.DataFrame:
        ...
        for transaction in self.transactions:
            for claim in transaction.claims:
                remits_dict = TransactionSet.serialize_claim(claim, transaction)[
                    "remits_dict"
                ]
    ...
    def build_remit_payers(self) -> pd.DataFrame:
        ...
        for transaction in self.transactions:
            for claim in transaction.claims:
                remit_payers_dict = TransactionSet.serialize_claim(claim, transaction)[
                    "remit_payers_dict"
                ]

                remit_payers_data.append(remit_payers_dict)

        remit_payers_data = pd.DataFrame(
            remit_payers_data, columns=remit_payers_data[0].keys()
        )
    ...
    def build_payment_fin_info(self) -> pd.DataFrame:
        ...
        for transaction in self.transactions:
            remit_financial_info_dict = TransactionSet.serialize_transaction(
                transaction
            )["remit_financial_info_dict"]

            remit_financial_info_df.append(remit_financial_info_dict)

        remit_financial_info_df = pd.DataFrame(
            remit_financial_info_df, columns=remit_financial_info_df[0].keys()
        )
            ...
            for claim in transaction.claims:
                for service in claim.services:
                    remit_service_lines_dict = TransactionSet.serialize_service(
                        transaction, claim, service
                    )["remit_service_lines_dict"]

                    remit_service_lines_df.append(remit_service_lines_dict)

        remit_service_lines_df = pd.DataFrame(
            remit_service_lines_df, columns=remit_service_lines_df[0].keys()
        )
    ...
    def build_remit_adjustments(self) -> pd.DataFrame:
        ...
        for transaction in self.transactions:
            for claim in transaction.claims:
                remit_adjustments_dict = TransactionSet.serialize_claim(
                    claim, transaction
                ...
                for service in claim.services:
                    for adjustment in service.adjustments:
                        service_line_adjustments_dict = (
                            TransactionSet.serialize_adjustment(
                                transaction, claim, service, adjustment
                            )["service_line_adjustments_dict"]
                        )
                ...
                for service in claim.services:
                    for remark in service.remarks:
                        remit_service_lines_remarks_dict = (
                            TransactionSet.serialize_service(
                                transaction, claim, service
                            )["remit_service_lines_remarks_dict"]
                        )
    ...
    def build_remit_remarks_adjudications(self) -> pd.DataFrame:
        ...
        for transaction in self.transactions:
            for claim in transaction.claims:
                remit_remarks_adjudications_dict = TransactionSet.serialize_claim(
                    claim, transaction
                )["remit_remarks_adjudications_dict"]
                remit_remarks_adjudications_data.append(
                    remit_remarks_adjudications_dict
                )
        remit_remarks_adjudications_data = pd.DataFrame(
            remit_remarks_adjudications_data,
    ...
    def build_provider_adjustments(self) -> pd.DataFrame:
        ...
        for transaction in self.transactions:
            provider_adjustment_dict = TransactionSet.serialize_transaction(
                transaction
            )["provider_adjustment_dict"]
            provider_adjustments_data.append(provider_adjustment_dict)

        provider_adjustments_data = pd.DataFrame(
            provider_adjustments_data, columns=provider_adjustments_data[0].keys()
        )
    ...
    @staticmethod
    def serialize_claim(
        claim: ClaimLoop, transaction: TransactionLoop
    ) -> dict[str, dict]:
        remits_dict = {
            ...
            if transaction.provider_summary
            else None,
            ...
            remit_remarks_adjudications_dict.update(
                {
                    ...
                    "MOA_nonpayable_professional_component_amount": claim.outpatient.non_payable_professional_component_amount,
                }
            )
    ...
    def serialize_transaction(transaction: TransactionLoop) -> dict[str, dict]:
        remit_financial_info_dict = {
            ...
            "payee_name": transaction.payee.name,
            "created_at": None,
            "tax_payee_id_qualifier": transaction.payee_identification.qualifier_code
            if transaction.payee_identification
            else None,
            "tax_payee_id": transaction.payee_identification.value
            if transaction.payee_identification
            else None,
            "payee_id_qualifier": transaction.payee.identification_code_qualifier,
            "payee_id": transaction.payee.identification_code,
            "other_payee_id_qualifier": transaction.other_payee_identification.qualifier_code
            if transaction.other_payee_identification
            else None,
    ...
    def serialize_service(
        ...
        remit_service_lines_dict = {
            ...
            if service.service_identification
            else None,
        ...
        return {
            "remit_service_lines_dict": remit_service_lines_dict,
            "remit_service_lines_remarks_dict": remit_service_lines_remarks_dict,
            "service_line_rendering_providers_dict": service_line_rendering_providers_dict,
        }
            ...
            service_line_adjustments_dict.update(
                {
                    ...
                    "remit_service_line_key": claim.claim.key
                    + "_"
                    + service.service.key,
