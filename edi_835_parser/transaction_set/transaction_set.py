...
class TransactionSet:
	...
	def build_payment_fin_info(self) -> pd.DataFrame:
		...
		for transaction in self.transactions:
			remit_financial_info_dict = TransactionSet.serialize_transaction(transaction)[
	...
	def build_remit_adjustments(self) -> pd.DataFrame:
		...
		for transaction in self.transactions:
			for claim in transaction.claims:
				remit_adjustments_dict = TransactionSet.serialize_claim(claim, transaction)[
			...
			for claim in transaction.claims:
				for service in claim.services:
					for remark in service.remarks:
						remit_service_lines_remarks_dict = TransactionSet.serialize_service(
							transaction, claim, service
	...
	def build_provider_adjustments(self) -> pd.DataFrame:
		...
		for transaction in self.transactions:
			provider_adjustment_dict = TransactionSet.serialize_transaction(transaction)[
	...
	@staticmethod
	def serialize_claim(claim: ClaimLoop, transaction: TransactionLoop) -> dict[str, dict]:
		remits_dict = {
